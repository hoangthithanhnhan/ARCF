    ...
    <header className="py-[40px] px-[72px]">
      <nav className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex space-x-[54px]">
            {["About", "How We Give", "Donations & Support", "News", "Contact"].map((item) => (
              <Link
                key={item}
                href="#"
                className="text-[1.25rem] font-semibold text-[var(--body)]"
              >
                {item}
              </Link>
            ))}
        ...
        <div>
          <Link
            href="#"
            className="bg-[var(--green)] text-[var(--body)] py-[13px] px-[50px] rounded-full font-semibold text-[1.25rem] no-underline"
          >
            Donate
          </Link>
        </div>
